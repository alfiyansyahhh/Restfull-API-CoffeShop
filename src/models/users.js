...
const usersModel = {
    ...
    insert: (body) =>{
        return new Promise((resolve, reject) => {
            db.query(
                `INSERT INTO users (
                username,password,picture,display_name,first_name,last_name,ttgl,gender,
                email_address,phone_number,delivery_address
                ) 
                VALUE (
                    '${body.username}','${body.password}','${body.picture}','${body.display_name}'
                    '${body.first_name}','${body.last_name}','${body.ttgl}','${body.gender}'
                    '${body.email_address}','${body.phone_number}','${body.delivery_address}',
                    )` , (err, result) => {
                if (err) {
                    reject(err)
                } else {
